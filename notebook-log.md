...
```shell
dna <- fasta2DNAbin(file="data/3loci_24samp_working/alignments/lsu/lsu_clustal_spname_9.fasta")
dna2 <- as.phyDat(dna)
```
...
Parsimony score for (lsu_clustal_spname_9.fasta): 572
...
Final pscore for lsu_clustal_spname_9.fasta: 566
Plot tree
...
- Parameters used:
    ...
    - burnin = 1 million generations (10%) (default setting for Mr Bayes)
...
Concatenation assumes that all genes within a species genome have undegone the same rate of evolution which is usually quite false. Incomplete Lineage Sorting (ILS), gene/genome duplication, and horizontal gene transfer are rather common occurences that lead to differences in how genes evolved.

The Multispecies Coalescent Model allows us to take the trees generated by a group of genes and infer the species tree.
...
```shell
cd /Users/hensley/Documents/coding_tools/phylogenetics_software/Astral
## basic run with astral preset parameters
java -jar astral.5.7.8.jar -i 3loci_concat_9.treefile -o 3loci_astral_9.treefile
## run with 1000 bootstrap replicates and the bootstrapping only performed with gene tree resampling
java -jar astral.5.7.8.jar -i 3loci_concat_9.treefile -o 3loci_astral_9_custom1.treefile -r 1000 --gene-only
```
